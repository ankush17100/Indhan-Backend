...
def login(request):
	if request.method=='POST':
		...
		hashPassword = encrypt(password,"blablabla")
		print("Atempt login with",username,password)
		try:
		...
		if user.password==hashPassword:
			...
			print("Some trouble with login", user)  
			return JsonResponse({
				'success':False,
			})
...
def signup(request):
	if request.method == "POST":
		...
		try:
			...
			print("User already exists",newuser)
			if newuser:
				return JsonResponse({
					'success':False,
			...
			newData.save()
			print("Create new user",userAccount)
			return JsonResponse({
				'success':True,
				'token':token
			})
...
def petrol_pump_ratings_response(request):
	if request.method =="POST":
		food = 0; bathroom = 0; cashless = 0; air = 0;
		name = request.POST['name']
		bathroom_bool = request.POST['bathroom']
		food_bool = request.POST['food']
		cashless_bool = request.POST['cashless']
		air_bool = request.POST['air']
		sanitation = int(float(request.POST['sanitation']))
		payment = int(float(request.POST['payment']))
		foodrating = float(request.POST['foodrating'])
		fraud = request.POST['fraud']
		if bathroom_bool:
			bathroom = 1
		if food_bool:
			food = 1
		...
		print("food",food)
		print("cashless",cashless)
		print("air",air)
		print("sanitation",sanitation)
		print("payment",payment)
		print("foodrating",foodrating)
		print("fraud",fraud)
		# try:
			
		petrol_pump = PetrolPump.objects.get(name=name)
		no = petrol_pump.number
		no += 1
		petrol_pump.number += 1
		petrol_pump.bathroom = (petrol_pump.bathroom*no + bathroom)/no
		petrol_pump.food = (petrol_pump.food*no + food)/no
		petrol_pump.cashless = (petrol_pump.cashless*no + cashless)/no
		petrol_pump.air = (petrol_pump.air*no + air)/no
		petrol_pump.sanitation = (petrol_pump.sanitation*no + sanitation)/no
		petrol_pump.payment = (petrol_pump.payment*no + payment)/no
		petrol_pump.foodrating = (petrol_pump.foodrating*no + foodrating)/no
		petrol_pump.rating = (petrol_pump.rating*no + (sanitation+payment+foodrating)/3)/no
		print('-------------------------------')
		print("bathroom",bathroom)
		print("food",petrol_pump.food)
		print("cashless",petrol_pump.cashless)
		print("air",petrol_pump.air)
		print("sanitation",petrol_pump.sanitation)
		print("payment",petrol_pump.payment)
		print("foodrating",petrol_pump.foodrating)
		# Save all data
		petrol_pump.save()
